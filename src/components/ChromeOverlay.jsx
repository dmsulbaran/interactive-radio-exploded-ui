...
// ─── Predefined SVG paths for social icons ───
const ICONS = {
...
// ─── SVG to 3D Extruded Icon Component ───
function Icon3D({ path, position, scale = 0.015, onClick }) {
  const shape = useMemo(() => {
    const loader = new SVGLoader()
    const svgData = loader.parse(`<svg xmlns="http://www.w3.org/2000/svg"><path d="${path}"/></svg>`)
  ...
  return (
    <mesh
      position={position}
      scale={[scale, -scale, scale]} // invert Y because SVGs draw top-down
...
export default function ChromeOverlay() {
  return (
    <group position={[-0.8, -0.6, 0]} rotation={[-0.1, 0.15, 0]} scale={0.2}>
      {/* 3D Text */}
      <Text3D
      ...
      <group position={[0.1, -0.5, 0]}>
        <Icon3D 
          path={ICONS.twitter} 
          position={[0, 0, 0]} 
          onClick={() => window.open('https://x.com/dmsulbaran', '_blank')}
        />
        <Icon3D 
          path={ICONS.linkedin} 
          position={[0.4, 0, 0]} 
          onClick={() => window.open('https://linkedin.com/in/dmsulbaran', '_blank')}
        />
        <Icon3D 
          path={ICONS.discord} 
          position={[0.8, 0, 0]} 
        ...
        <Icon3D 
          path={ICONS.github} 
          position={[1.2, 0, 0]} 
          onClick={() => window.open('https://github.com/dmsulbaran', '_blank')}
        />
      </group>
    </group>
  )
